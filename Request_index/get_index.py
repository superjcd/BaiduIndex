...
class BaiduIndex:
    ...
    def __init__(self, keywords, start_date, end_date):
        self._keywords = keywords if isinstance(
            keywords, list) else keywords.split(',')
        self._time_range_list = self.get_time_range_list(start_date, end_date)
        self._all_kind = ['all', 'pc', 'wise']
        self.result = {keyword: defaultdict(list)
    ...
    def get_result(self):
        ...
        for start_date, end_date in self._time_range_list:
            encrypt_datas, uniqid = self.get_encrypt_datas(
                # encrpt_datas其实就是userIndexes的列表
                start_date, end_date)
    ...
    def get_encrypt_datas(self, start_date, end_date):
        ...
        print('url for get_encrypt data is {}'.format(url))
    ...
    def decrypt_func(key, data):
        ...
        decrypt data
        """
        a = key  # 基于get_key(uniqid)获得的key
        i = data  # 给予get_encry_data获取的加密字符串
        n = {}
        s = []
...
if __name__ == '__main__':
    ...
    for data in baidu_index('黄渤', 'all'):
        print(data)
